...
exports.run = async (client, message, args) => {
  ...
  let mesaj = args.slice(0).join(' ')
      ...
      if (!mesaj) {
        return message.channel.send(`<:CARRno:524246262191751168> Çıkış mesajını yazmalısın.`)
    }
  
    db.set(`cikism_${message.guild.id}`, mesaj)
    message.channel.send(`<:CARRyes:524245926358024193> Çıkış mesajı \`${mesaj}\` olarak ayarlandı.`)
}
