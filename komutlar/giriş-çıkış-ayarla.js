...
exports.run = async (client, message, args) => {
  ...
  let channel = message.mentions.channels.first()
    ...
    if (!channel) {
        return message.channel.send(`<:CARRno:524246262191751168> Giriş-çıkış mesaj kanalı olarak ayarlamak istediğin kanalı etiketlemelisin.`)
    }
  
  if (!channel) return;
  
    db.set(`mgcK_${message.guild.id}`, channel.name)
    message.channel.send(`<:CARRyes:524245926358024193> Giriş-çıkış mesaj kanalı başarıyla \`#${channel.name}\` olarak ayarlandı.`)
