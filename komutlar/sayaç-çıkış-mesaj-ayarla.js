...
exports.run = async (client, message, args) => {
  ...
  let mesaj = args.slice(0).join(' ')
      ...
      if (!mesaj) {
        return message.channel.send(`<:CARRno:524246262191751168> Sayaç çıkış mesajını yazmalısın.`)
    }
  
    db.set(`sayaccm_${message.guild.id}`, mesaj)
    message.channel.send(`<:CARRyes:524245926358024193> Sayaç çıkış mesajı \`${mesaj}\` olarak ayarlandı.`)
}
