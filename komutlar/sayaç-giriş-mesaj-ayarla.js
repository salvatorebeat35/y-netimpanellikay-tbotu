...
exports.run = async (client, message, args) => {
  ...
  let mesaj = args.slice(0).join(' ')
      ...
      if (!mesaj) {
        return message.channel.send(`<:CARRno:524246262191751168> Sayaç giriş mesajını yazmalısın.`)
    }
  
    db.set(`sayacgm_${message.guild.id}`, mesaj)
    message.channel.send(`<:CARRyes:524245926358024193> Sayaç giriş mesajı \`${mesaj}\` olarak ayarlandı.`)
}
